...
class Runner(Node):
    ...
    def manipulate_scene(self, child):
        self.sms_request.command = 'clone'
        self.sms_request.child_frame_id = child
        self.sms_request.parent_frame_id = "floor"
        self.sms_request.new_frame_id = "locked_" + child
        self.persist = False
        self.sms_future = self.sms_client.call_async(self.sms_request)
        self.sms_future.add_done_callback(partial(self.sms_future_callback))
    ...
    def lock_arucos(self):
        run: bool = self.state.get('lock_run')
        if run and not self.lock_done:
    ...
    # TODO: this has to be djusted to the lab, i.e. to be more genral so that we can also sent in which tcp:
    # def send_sms_request(self):
    #     run: bool = self.state.get('gripper_run')
    #     grip_cmd = self.state.get('gripper_command')
    #     if run:
    #         if grip_cmd == "pick_red":
    #             self.pick("red_cube")
    #         if grip_cmd == "pick_green":
    #             self.pick("green_cube")
    #         if grip_cmd == "pick_blue":
    #             self.pick("blue_cube")
    #         if grip_cmd == "place_red":
    #             self.place("red_cube")
    #         if grip_cmd == "place_green":
    #             self.place("green_cube")
    #         if grip_cmd == "place_blue":
    #             self.place("blue_cube")
    #     else:
    #         self.upd_state('gripper_command', "none")
    ...
    #     )
    #     if self.sms_response.success: # type: ignore
    #         self.upd_state('gripper_command', "done")
