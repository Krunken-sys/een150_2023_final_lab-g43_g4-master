...
def the_model() -> Model:
    ...
    initial_state = State(
        ...
        goal_as_string = "robot_pose == pose_1",
        replan = False,
        lock_run = False,
    ...
    ops[f"move_to_pose_1"] = Operation (
        ...
        precondition = Transition("pre",
            g(f"!robot_run && robot_state == initial && robot_pose != pose_1 && robot_pose == above_pose_1"),                
    ...
    ops[f"move_to_pose_2"] = Operation (
        ...
        precondition = Transition("pre",
            g(f"!robot_run && robot_state == initial && robot_pose != pose_2 && robot_pose == above_pose_2"),                
    ...
    # ops[f"move_to_pose_3"] = Operation (
    #     name=f"move_to_pose_3",

    #     precondition = ("pre",
    #         g(f"!robot_run && robot_state == initial && robot_pose != pose_3 && robot_pose == above_pose_3"),                
    #         a(f"robot_command = move_j, robot_run, robot_goal_frame = pose_3")),

    #     postcondition = ("post",
    #         g(f"robot_state == done"),
    #         a(f"!robot_run, robot_pose <- pose_3")),
    #     effects = (),
    #     to_run = Transition.default()
    #     )
    # move to above_table
    ...
    ops[f"move_to_above_pose_1"] = Operation (
        ...
        precondition = Transition("pre",
            g(f"!robot_run && robot_state == initial && robot_pose != above_pose_1"),                
    ...
    ops[f"move_to_above_pose_2"] = Operation (
        name=f"move_to_above_pose_2",

        precondition = Transition("pre",
            g(f"!robot_run && robot_state == initial && robot_pose != above_pose_2"),                
            a(f"robot_command = move_j, robot_run, robot_goal_frame = above_pose_2")),

        postcondition = Transition("post",
            g(f"robot_state == done"),
            a(f"!robot_run, robot_pose <- above_pose_2")),
        effects = (),
        to_run = Transition.default()
        )    
    ...
    ops[f"pick_at_pose_1"] = Operation (
        name=f"pick_at_pose_1",

        precondition = Transition("pre",
            g(f"!suction_cup_1_occ && !robot_run && robot_state == initial && robot_pose == pose_1 && cyl_at_pose_1"),                
            a(f"robot_command = pick, robot_run")),

        postcondition = Transition("post",
            g(f"robot_state == done"),
            a(f"!robot_run, robot_pose <- pose_1, suction_cup_1_occ, !cyl_at_pose_1")),
        effects = (),
        to_run = Transition.default()
        )
    ...
    ops[f"pick_at_pose_2"] = Operation (
        name=f"pick_at_pose_2",

        precondition = Transition("pre",
            g(f"!suction_cup_1_occ && !robot_run && robot_state == initial && robot_pose == pose_2 && cyl_at_pose_2"),                
            a(f"robot_command = pick, robot_run")),

        postcondition = Transition("post",
            g(f"robot_state == done"),
            a(f"!robot_run, robot_pose <- pose_2, suction_cup_1_occ, !cyl_at_pose_2")),
        effects = (),
        to_run = Transition.default()
        )
    ...
    ops[f"place_at_pose_1"] = Operation (
        name=f"place_at_pose_1",
    ...
    ops[f"place_at_pose_2"] = Operation (
        name=f"place_at_pose_2",
